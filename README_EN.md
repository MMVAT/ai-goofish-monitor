...
1. Open the default Web UI at `http://127.0.0.1:8018` and sign in.
...
- Default Web UI: `http://127.0.0.1:8018`
...
python -m src.app
# or
uvicorn src.app:app --host 0.0.0.0 --port 8018 --reload
...
- The Vite dev server proxies `/api`, `/auth`, and `/ws` to `http://127.0.0.1:8018`.
...
- `./start.sh` prints the default app URL `http://localhost:8018` and API docs URL `http://localhost:8018/docs`.
...
- `SERVER_PORT`: backend port, default `8018`.
